...
\title{Canny-\underline{RX}: Selective Canny Control and \underline{R}egional Te\underline{x}t Guidance for Training-free Image {E}diting}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.02\linewidth]{figures/figure_tradeoff2.png}
...
%\item Performance. Compared to previous training-free editing and training-based methods, our method share a significantly higher text adherence performance on the real-world image editing involving complex interaction with context (\% improvement for adding, removal and replacement respectively over current best Open-source methods), and comparable level of context preservation in adding, removal, and replacenment task. In terms of editing seamlessness, when paired with our editing results with the ground-truth editing images, we ask which image is more likely to be AI-edited, the users hardly random pick while users pick results of other methods as AI-edited easily.
...
\begin{table*}[h]
\begin{center} 
\setlength{\tabcolsep}{1.60mm} %
\caption{\textbf{Automatically computed metrics comparing with representative methods on RICE-Bench.}. RF Edit and KV-Edit are training-free flow-based methods. The inject denotes the number of denoising steps where attention feature injection is applied. BrushEdit, Powerpaint-Flux and FLUX Fill represent training-based methods. The metrics are Context Preservation(CP) and Text Adherence(TA). TO Score measures the trade-off for editabily and fidelity, which sums up CP and TA across 3 editing tasks. \textbf{Bold} and \underline{underlined} values represent the best and second-best results respectively.}
...
\midrule
RF Edit($\textup{inject}=2$)~\cite{wang2024taming} & 71.77 & 22.65 & 42.99 & \textbf{39.05} & 47.44 & 9.22 & 233.1\\
RF Edit($\textup{inject}=8$)~\cite{wang2024taming} & \textbf{99.13} & 2.14 & \textbf{79.34} & 2.94 & \textbf{67.89} & 5.03 & 256.5\\
...
\caption{\textbf{Comparison with other methods on PIE-Bench.} VAE$^*$ denotes the inherent reconstruction error through VAE reconstruction only. P2P and MasaCtrl are DDIM-based methods, while RF Inversion, RF Edit and KV-Edit are flow-based. BrushEdit, FLUX fill and Powerpaint-Flux represent training-based methods. Except result of ours, other results follow \cite{zhu2025kv}. \textbf{Bold} and \underline{underlined} values represent the best and second-best results respectively.}
...
RF Edit~\cite{wang2024taming} & \underline{27.60} & \underline{6.56}& 24.44& 113.20& 56.26& 22.08& 5.18\\
