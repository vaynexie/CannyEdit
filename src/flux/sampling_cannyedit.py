...
def denoise_cannyedit(
    ...
    for t_curr, t_prev in tqdm(zip(timesteps[:-1], timesteps[1:]), total=len(timesteps) - 1,
        ...
        if i==0:
            ...
            if inversion_save_path!=None and (stage == 'stage_refine' or stage=='stage_generate_regen'):
        ...
        apply_local_point = 0.75
        apply_extenda_point= 0.6
        if inversion_save_path!=None and 'stage_generate' in stage:apply_extenda_point= 0.8
        ...
        if i >= time_to_start:
            if i == time_to_start:
                if inversion_save_path!=None and 'stage_generate' in stage:
                    ...
                    img = info[timesteps[i+3]]
                else:
                    img = info[timesteps[i+1]]
                print('seed')
                print(seed)
                ...
                if  'stage_generate' in stage:
                    ...
                    prev_generate = info_generate[i]
                    img[:, local_mask1_flat.bool(), :] = prev_generate[:, local_mask1_flat.bool(), :]
                    for local_mask2_flat in local_mask_add_flat:
                        img[:, local_mask2_flat.bool(), :] = prev_generate[:, local_mask2_flat.bool(), :]
